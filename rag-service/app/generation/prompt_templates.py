"""Prompt template helpers for RAG generation."""
...
def build_compare_prompt(
    ...
    return (
        "你是一个专业的文档问答助手。\n"
        f"当前使用的是“{mode_desc}”模式。\n"
        "请基于以下检索到的内容回答问题，并在回答中使用 [1], [2] 等标记引用对应结果。\n\n"
        f"检索内容：\n{context}\n\n"
        f"问题：{query}\n\n"
        "请给出详细、准确的回答。"
    )
...
def _build_zh_prompt(query: str, chunks: list[RetrievalResult], history: list[dict] | None) -> str:
    ...
    system = (
        "你是一个专业的文档问答助手。你的任务是基于给定的检索内容回答用户问题。\n"
        "规则：\n"
...
def _format_chunks(chunks: list[RetrievalResult]) -> str:
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        content = normalize_extracted_text(chunk.content)
